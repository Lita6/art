...
(game_memory *Memory, game_offscreen_buffer *Buffer)
{
	...
	(void)Black;
	Color Red = make_color(255.0, 0.0, 0.0);
	(void)Red;
	Color Green = make_color(0.0, 255.0, 0.0);
	(void)Green;
	Color Blue = make_color(0.0, 0.0, 255.0);
	(void)Blue;
	Color Yellow = make_color(255.0, 255.0, 0.0);
	(void)Yellow;
	Color White = make_color(255.0, 255.0, 255.0);
	(void)White;
	Color SkyBlue = make_color(135.0, 205.0, 235.0);
	(void)SkyBlue;
...
#define LightCount 3
	...
	for(s32 y = ScreenMinY; y < ScreenMaxY; y++)
	{
		for(s32 x = ScreenMinX; x < ScreenMaxX; x++)
		{
			...
			r32 tMin = ViewDistanceFromCamera;
			...
			for(s32 i = 0; i < 2; i++)
			{
				...
				if(reflections[0].intersection.z > 5.0)
				{
					s32 a = 0;
					a = 5*8;
				}
